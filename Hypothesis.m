...
function [projMatch,onsetMatch] = closestPairs(proj,onsets)
    ...
    projMatch = zeros(1,min(projN,onsetsN));
    onsetsMatch = zeros(1,min(projN,onsetsN));
    for i = 1:projN
        best = pair(i);
        bestDist = dist(i);
        if onsetsMatch(end) ~= pair(i)                     % TODO: WIll out of bounds for 1
            projMatch(end+1) = i;
            onsetsMatch(end+1) = pair(i);
        end
    end
end
